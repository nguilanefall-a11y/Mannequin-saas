...
const CONTRACT_TYPES = [
    ...
    { id: 'image', title: 'Contrat de Cession Image', desc: 'Cession de droits sur une durée déterminée.', icon: '🌟' },
    { id: 'exclusif', title: 'Contrat de Représentation Exclusive', desc: 'Mandat exclusif agence/mannequin.', icon: '🔒' },
];

export default function ContractWizard({ models, clients }: { models: any[], clients: any[] }) {
            ...
            {models.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {models.map(model => (
                        ...
                        >
                            <div className={`w-20 h-20 rounded-full bg-gray-100 flex items-center justify-center text-2xl font-bold mb-4 group-hover:scale-110 transition-transform`}>
                                {model.prenom.charAt(0)}
                            </div>
                            <span className="font-bold text-lg text-gray-900">{model.prenom} {model.nom}</span>
                ...
                <div className="text-center py-20 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200">
                    <p className="text-gray-500 mb-4">Aucun modèle disponible.</p>
                    <Link href="/models/new" className="bg-black text-white px-6 py-2 rounded-lg font-bold hover:bg-gray-800">Ajouter un Modèle</Link>
                </div>
            )}
        </div>
    );

    // STEP 3: CLIENT (REAL DATA)
    const renderStep3 = () => (
        <div className="animate-in slide-in-from-right duration-300">
            <div className="flex items-center gap-3 mb-6">
            ...
            {clients.length > 0 ? (
                <div className="space-y-4">
                    {clients.map(client => (
                        ...
                        >
                            <div>
                                <h3 className="font-bold text-gray-900">{client.nomSociete}</h3>
                ...
                <div className="text-center py-20 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200">
                    ...
                    <Link href="/clients/new" className="bg-black text-white px-6 py-2 rounded-lg font-bold hover:bg-gray-800">Ajouter un Client</Link>
                </div>
            )}
        </div>
    );
        ...
        <div className="animate-in slide-in-from-right duration-300 max-w-xl mx-auto">
            ...
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-200 space-y-6">
                ...
                <button
                    ...
                    className="w-full py-4 bg-black text-white font-bold rounded-xl text-lg hover:bg-gray-800 transition-all shadow-lg shadow-gray-900/10 mt-4 cursor-pointer"
                >
                    Générer le Document
                </button>
            </div>
        ...
        <div className="animate-in zoom-in-95 duration-500">
            <div className="flex justify-between items-center mb-8 no-print">
                ...
                <div className="flex gap-3">
                    ...
                    <button onClick={() => window.print()} className="px-4 py-2 text-sm font-medium bg-black text-white hover:bg-gray-800 rounded-lg shadow-sm cursor-pointer">Imprimer / PDF</button>
                </div>
            </div>
            ...
            <div className="bg-white w-full max-w-4xl mx-auto p-16 shadow-2xl rounded-sm border border-gray-100 min-h-[900px] flex flex-col font-serif text-justify leading-relaxed text-gray-900 relative">
                ...
                <div className="space-y-6 mb-10">
                    ...
                    <p className="pl-6">
                        <strong>{selectedModel?.prenom} {selectedModel?.nom}</strong>, demeurant au {selectedModel?.adresse || 'Adresse non renseignée'}.<br />
                    ...
                    {contractType.id !== 'exclusif' && (
                        <>
                            ...
                            <p className="pl-6">
                                <strong>{selectedClient?.nomSociete}</strong>, dont le siège est situé à {selectedClient?.adresse}.<br />
                ...
                <div className="mt-20 flex justify-between items-start font-sans">
                    ...
                    <div className="text-center w-1/3">
                        ...
                        <div className="font-dancing-script text-2xl text-black mt-4 font-normal">{selectedModel?.prenom} {selectedModel?.nom}</div>
                    </div>
                    {contractType.id !== 'exclusif' && (
                        <div className="text-center w-1/3">
